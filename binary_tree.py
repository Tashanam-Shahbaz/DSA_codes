...
class BTree:
    ...
    #         return True
    #     return False
    def isPerfect(self,h,level=0):
        if self.lc is None and self.rc is None:
            return h == level + 1
        if self.lc is None or self.rc is None:
            return False
        return self.lc.isPerfect(h,level+1) and self.rc.isPerfect(h,level+1)
    ...
    def buildtree(self, val):
        ...
        n = 0  # keeps track of item in nodes
        i = 0  # keeps track of item in val
        while i < (len(val) - 1):
            p = nodes[n]
            # print(i)
            # print(val[i])
    ...
    def heap(self):
        if not self.isAlmostComplete():
            return False
        lst=[self]
        for i in range(1,len(self.nodecount())):
            lst.append(None)
        for j in range(0, len(self.nodecount())):
            lst[2j+1]=self.lc
            lst[2j+1]=self.rc


val=[8,2,3,6,2,7,2,3,4,5,1,2,3,4]
n = BTree(5)
n.buildtree(val)
n.traversebf()
print(n.isPerfect(n.height()))
# def maxheap(lst, val):
#     lst.append(val)
#     indc = len(lst) - 1
#     indp = (indc - 1) // 2
#     while lst[indc] > lst[indp] and not indc < 0:
#         lst[indc], lst[indp] = lst[indp], lst[indc]
#         indc=indp
#         indp=(indc-1)//2
#     return lst
# # print(maxheap([100,84,71,60,23,12,29,1,37,4],30))
